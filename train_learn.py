    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        if(configs.sparse.bp_data_alg == "smd"):
            if np.random.rand() < float(configs.sparse.bp_data_sparsity):
                continue
        # if(batch_idx < len(train_loader)*configs.sparse.bp_data_sparsity):
        #     continue
        ...
        if(teacher is not None and configs.sparse.bp_data_alg == "is" and float(configs.sparse.bp_data_sparsity) > 0):
            with torch.no_grad():
                teacher_score = F.softmax(teacher(data.data), dim=-1)

                # [bs]\in [0,c]
                teacher_entropy = teacher_score.mul(
                    -(teacher_score+1e-12).log()).sum(dim=-1)
            # print(teacher_entropy)
            mask = teacher_entropy > torch.quantile(
                teacher_entropy, float(configs.sparse.bp_data_sparsity))  # difficult
            # mask = teacher_entropy.clone().bool().bernoulli_(p=0.2)
            # mask = teacher_entropy < torch.quantile(teacher_entropy, 0.7) # confident
            # print(mask.float().sum().item())
            if(not mask.any()):
                continue
            data = data[mask, :]
            target = target[mask]
            ...
            # classify_loss = distillation_loss(output, teacher_score, target, T=1, alpha=0.9)
        else:
            output = model(data)
            classify_loss = criterion(output, target)
...
def main() -> None:
    ...
    model = builder.make_model(device, int(configs.run.random_state) if int(configs.run.deterministic) else None)
    ...
    try:
        ...
        if(int(configs.checkpoint.imagenet_pretrain)):
            import torchvision
