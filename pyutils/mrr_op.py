...
#       mrr_op      #
#####################
from .compute import complex_mult, complex_to_polar, get_complex_energy, get_complex_magnitude, polar_to_complex, polynomial
...
__all__ = ["mrr_voltage_to_delta_lambda", "mrr_tr_to_roundtrip_phase", "mrr_roundtrip_phase_to_tr", "mrr_roundtrip_phase_to_tr_fused", "mrr_roundtrip_phase_to_tr_grad_fused", "mrr_roundtrip_phase_to_tr_func", "mrr_roundtrip_phase_to_out_phase",
           "mrr_tr_to_out_phase", "mrr_roundtrip_phase_to_tr_phase", "mrr_roundtrip_phase_to_tr_phase_fused", "mrr_modulator", "mrr_filter", "morr_filter", "mrr_fwhm_to_ng", "mrr_ng_to_fsr", "mrr_finesse"]
...
def mrr_roundtrip_phase_to_tr(rt_phi, a: float = 0.8, r: float = 0.9, poly_coeff=None, intensity: bool = False):
    '''
    ...
    if(poly_coeff is not None):
        ...
        # ###  Since PyTorch 1.7 rsub is not supported for autograd of complex, so have to use negate and add
        # a_ephi = -a * ephi
...
def mrr_roundtrip_phase_to_tr_grad_fused(rt_phi, a: float = 0.8, r: float = 0.9, intensity: bool = False):
    ...
    '''
    if(not intensity):
        g = (a*r*(a**2 - 1)*(r**2 - 1)*rt_phi.sin())/((a**2 + r**2 - 2*a*r*rt_phi.cos())**(1/2)*(a**2*r**2 + 1 - 2*a*r*rt_phi.cos())**1.5)
    else:
        g = ((a**2 - 1)*(r**2 - 1)*2*a*r*rt_phi.sin())/(a**2*r**2 + 1 - 2*a*r*rt_phi.cos())**2
    return g
