...
import numpy as np
import torch
from .mrr_op import mrr_roundtrip_phase_to_tr_fused
__all__ = ["quant_kaiming_uniform", "quant_kaiming_uniform_", "truncated_normal", "truncated_normal_", "morr_uniform_", "morr_uniform"]
...
def morr_uniform_(tensor, MORRConfig, n_op=4, biased=False, gain=1):
    ...
    return {}
    '''
    morr_fwhm = -4 * np.pi**2 * MORRConfig.radius * MORRConfig.effective_index * (1/MORRConfig.resonance_wavelength-1/(MORRConfig.resonance_wavelength - MORRConfig.bandwidth/2))
