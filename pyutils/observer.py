'''
@Description: Copied from https://github.com/pytorch/pytorch/blob/46bd76fdecef5afe3caa23446645273e80278d39/torch/quantization/observer.py
                such that we can use in pytorch1.6
More: pytorch 1.6 not supported per_channel_affine_float_qparams, torch.quint4x2, torch._aminmax
'''
import warnings
...
class PerChannelMinMaxObserver(_ObserverBase):
    ...
    def _forward(self, x_orig):
        ...
        if min_vals.numel() == 0 or max_vals.numel() == 0:
            ...
            min_vals = torch.min(y, 1)
            max_vals = torch.max(y, 1)
        else:
            # min_vals_cur, max_vals_cur = torch._aminmax(y, 1)
            min_vals_cur = torch.min(y, 1)
            max_vals_cur = torch.max(y, 1)
