...
__all__ = ["uniform_quantize_cpu", "pact_quantize", "PACT_Act", "uniform_quantize", "input_quantize_fn", "weight_quantize_fn"]
...
class input_quantize_fn(torch.nn.Module):
    def __init__(self, in_bit,  alg="dorefa", device=torch.device("cuda:0"), quant_ratio=1.0):
        ...
        if(torch.__version__ < "1.7"):
            self.obs = MovingAverageMinMaxObserver(averaging_constant=0.01, dtype=torch.quint8,
            qscheme=torch.per_tensor_affine, reduce_range=False, quant_min=0, quant_max=2**self.in_bit-1).to(self.device)
        else:
            self.obs = torch.quantization.observer.MovingAverageMinMaxObserver(averaging_constant=0.01, dtype=torch.quint8,
            qscheme=torch.per_tensor_affine, reduce_range=False, quant_min=0, quant_max=2**self.in_bit-1).to(self.device)

    def set_quant_ratio(self, quant_ratio=None):
        if(quant_ratio is None):
            ### get recommended value
            quant_ratio = [None, 0.2, 0.3, 0.4, 0.5, 0.55, 0.6, 0.7, 0.8, 0.83, 0.86, 0.89, 0.92, 0.95, 0.98, 0.99, 1][min(self.in_bit, 16)]
        assert 0 <= quant_ratio <= 1, logging.error(f"Wrong quant ratio. Must in [0,1], but got {quant_ratio}")
        self.quant_ratio = quant_ratio
...
class weight_quantize_fn(torch.nn.Module):
    def __init__(self, w_bit, mode="oconv", alg="dorefa", quant_ratio=1.0):
        ...
        self.alg = alg
        self.mode = mode
        assert alg in {"dorefa", "dorefa_sym", "qnn", "dorefa_pos"}, logging.error(f"Only support (dorefa, dorefa_sym, qnn, dorefa_pos) algorithms, but got {alg}")
        self.quant_ratio = quant_ratio
        assert 0 <= quant_ratio <= 1, logging.error(f"Wrong quant ratio. Must in [0,1], but got {quant_ratio}")
    ...
    def set_quant_ratio(self, quant_ratio=None):
        if(quant_ratio is None):
            ### get recommended value
            quant_ratio = [None, 0.2, 0.3, 0.4, 0.5, 0.55, 0.6, 0.7, 0.8, 0.83, 0.86, 0.89, 0.92, 0.95, 0.98, 0.99, 1][min(self.w_bit, 16)]
        assert 0 <= quant_ratio <= 1, logging.error(f"Wrong quant ratio. Must in [0,1], but got {quant_ratio}")
        self.quant_ratio = quant_ratio
...
class PACT_Act(torch.nn.Module):
    ...
    def __init__(self, precision=None, alpha=1., backprop_alpha=True, statistics_only=False, leaky=None, device=torch.device("cuda")):
