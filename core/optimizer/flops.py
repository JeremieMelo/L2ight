...
class FLOPSOptimizer(Optimizer):
    ...
    def perturb(self, params, sigma):
        perturbs = {layer_name: {p_name: self._sample_perturbation(
            p, sigma) for p_name, p in layer_params.items()}for layer_name, layer_params in params.items()}
        perturbed_params = {layer_name: {p_name: p+perturbs[layer_name][p_name]
                                         for p_name, p in layer_params.items()}for layer_name, layer_params in params.items()}
        ...
        for layer_name, layer in self.modules.items():
            for param_name, phases in params[layer_name].items():
                if(param_name == "phase_U"):
                    ...
                    layer.S.data.copy_(phases.data.cos().view_as(
                        layer.S).mul_(layer.S_scale))
                else:
    ...
    def _compute_gradient(self, loss_diff, perturb, sigma):
        ...
        return {layer_name: {p_name: p.mul(loss_diff) for p_name, p in layer_params.items()} for layer_name, layer_params in perturb.items()}

    def _accumulate_gradient(self, buffer, grad, scale):
        return {layer_name: {p_name: p.add_(grad[layer_name][p_name]*scale) for p_name, p in layer_params.items()} for layer_name, layer_params in buffer.items()}

    def _apply_gradients(self, params, grad, lr):
        return {layer_name: {p_name: p.sub_(grad[layer_name][p_name]*lr) for p_name, p in layer_params.items()} for layer_name, layer_params in params.items()}
    ...
    def zo_gradient_descent(self, obj_fn, params):
        ...
        for _ in range(self.n_sample):
            ...
            grads = grad if grads is None else self._accumulate_gradient(
                grads, grad, 1/self.n_sample)
