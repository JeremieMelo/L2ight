...
LastEditTime: 2021-10-24 16:35:39
'''
import os
import sys
...
from torchonn.op.mzi_op import (checkerboard_to_vector,
...
class LinearFeatureSampler(torch.nn.Module):
    ...
    def set_sparsity(self, sparsity: float, random_state: Optional[int] = None) -> None:
        assert 0 <= sparsity <= 1, logger.error(f"Illegal sparsity, must within [0,1] but got {sparsity}.")
        self.sparsity = sparsity
        self.random_state = random_state
    ...
    def reconstruct(self, x: Tensor, mask: Optional[Tensor] = None) -> Tensor:
        ...
        if mask is None:
            return x
        out = torch.zeros(self.input_size, device=x.device).view(-1, self.n_block, self.miniblock)
        out[mask, :] = x
        out = out.flatten(1)
        return out
...
class Conv2dFeatureSampler(torch.nn.Module):
    ...
    def set_sparsity(
        self, spatial_sparsity: float, column_sparsity: float, random_state: Optional[int] = None
    ) -> None:
        ...
        if self.kernel_size == 1:  # merge column sampling to spatial sampling to save memory
            self.spatial_sparsity = 1 - (1 - self.spatial_sparsity) * (1 - self.column_sparsity)
    ...
    def sample(self, x: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        if self.normalize in {"exp", "var"}:
            ...
            self.scaling_factor = 1 / ((1 - real_spatial_sparsity) * (1 - real_column_sparsity) + 1e-12)
...
class FeedbackSampler(torch.nn.Module):
    ...
    def __init__(
        ...
        self.alg = alg
        assert alg in self.__alg_list__, logger.error(f"Only support {self.__alg_list__}, but got {alg}.")
        self.normalize = normalize
        self.mode = mode
        assert mode in self.__mode_list__, logger.error(f"Only support {self.__mode_list__}, but got {mode}.")
    ...
    def set_sparsity(
        self, forward_sparsity: float, backward_sparsity: float, random_state: Optional[int] = None
    ) -> None:
    ...
    def sample_topk(self, x: Tensor, mask: Tensor, forward: bool = False) -> Tensor:
        ...
        if mask is None:
            mask = self.mask = torch.ones(x.size(0), x.size(1), device=x.device, dtype=torch.bool)
        ...
        if forward:
            thres = torch.quantile(norm, q=sparsity, dim=1, keepdim=True)  # forward: [p, 1]
        else:
            thres = torch.quantile(norm, q=sparsity, dim=0, keepdim=True)  # backward: [1, q]
    ...
    def sample(self, x: Tensor, mask: Tensor = None, forward: bool = False) -> Tensor:
        ...
        if self.alg == "uniform":
            ...
            raise NotImplementedError(f"Only support {self.__alg_list__}, but got {self.alg}.")
...
class SingularValueGradientSampler(torch.nn.Module):
    ...
    def topk_mask(self, x: Tensor) -> Tensor:
        if self.rank < x.size(-1):
            indices = torch.topk(x.abs(), k=self.rank, dim=-1, largest=True, sorted=False)[1]
            self.mask = torch.zeros_like(x, dtype=torch.bool).scatter_(-1, indices, 1)
        else:
            self.mask = None
        return self.mask
...
class LearningProfiler(torch.nn.Module):
    ...
    def update_forward(
        self, x: Tensor, weight: Tensor, output: Tensor, feedback_sampler: FeedbackSampler
    ) -> None:
        ...
        self.report["forward_core_call"] += active_weight_block * batch  # p*q*sparsity*batch
            ...
            if isinstance(feature_sampler, Conv2dFeatureSampler):
                if feature_sampler.kernel_size == 1:  # conv1x1: spatial sampling=column sampling
                    ...
                    if feature_sampler.column_sparsity > 1e-9:
                        active_column = feature_sampler.column_mask.float().sum().item() * x.size(
                            0
                ...
                # nonzero * p
                self.report["backward_weight_core_call"] += mask.sum().item() * weight.size(0)
                self.report["backward_weight_accum_step"] += mask.sum(0).max().item() * x.size(0)
        # input backward
        if x_requires_grad:
            ...
            if feedback_sampler.forward_sparsity > 1e-9 or feedback_sampler.backward_sparsity > 1e-9:
                ...
                if x.dim() == 2:  # fully-connected
                    max_accum_step = max(0, feedback_sampler.mask.sum(0).max().item() - 1)  # p
                ...
                if x.dim() == 2:  # fully-connected
                    ...
                    max_accum_step = np.ceil(x.size(1) / p) * np.ceil(np.log2(2 * k)) * np.ceil(p / 2)
                elif x.dim() == 4 and feature_sampler.kernel_size == 1:  # conv1x1
                    max_accum_step = max(0, weight.size(0) - 1)  # p-1
            if x.dim() == 2:  # linear
                batch = x.size(0)
    ...
    def update(
        ...
        if feedback_sampler.forward_sparsity > 1e-9:
            ...
            active_weight_block = weight.size(0) * weight.size(1)
            max_accum_step = weight.size(1) - 1
        batch = x.size(0) if x.dim() == 2 else int(grad_output.numel() / grad_output.size(1))
        self.report["forward_core_call"] += active_weight_block * batch  # p*q*sparsity*batch
        # do not forget we do im2col sequentially
        self.report["forward_accum_step"] += max_accum_step * batch + np.ceil(
            batch / k
...
def test_singularvalue_sampler():
    ...
    print(grad_s)
    print(u.transpose(-1, -2).matmul(grad_weight.matmul(v.transpose(-1, -2)))[..., :: k + 1])
