...
Date: 2021-10-24 16:32:47
LastEditors: Jiaqi Gu (jqgu@utexas.edu)
LastEditTime: 2021-10-24 16:32:47
"""
...
def sparse_bp_linear(
    ...
    feature_sampler: LinearFeatureSampler = None,
    rank_sampler: SingularValueGradientSampler = None,
    profiler: LinearFeatureSampler = None,
) -> Tensor:
    ...
    class SparseBPLinearFunction(torch.autograd.Function):
        ...
        def forward(ctx, x: Tensor, u: Tensor, s: Tensor, v: Tensor) -> Tensor:
            ...
            with torch.no_grad():
                ...
                out = F.linear(x, weight.permute([0, 2, 1, 3]).contiguous().view(outc, inc), bias=None)
        ...
        def backward(ctx, grad_output: Tensor) -> Tuple[Tensor, ...]:
            ...
            if ctx.needs_input_grad[0]:
                ...
                grad_input = grad_output.matmul(weight.permute([0, 2, 1, 3]).contiguous().view(outc, inc))
            ...
            if ctx.needs_input_grad[2]:
                ...
                grad_weight = grad_weight.view(p, k, q, k).permute([0, 2, 1, 3])  # [p, q, k, k]

                # x   -> (V*_noisy -> I -> I_noisy)  -> yv
                # dys <- (I_noisy* <- I <- U^*_noisy) <- dy
                # u^T x dw = [p, q, k, k] or [p, q, bp_rank, k]
                if rank_sampler is not None:
                    grad_sigma = rank_sampler.sample(u, s, v, grad_weight, I_U, I_V)
...
class MZIBlockLinear(torch.nn.Module):
    ...
    def set_bp_input_sampler(
        self, sparsity: float, normalize: str = "none", random_state: Optional[int] = None
    ):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if isinstance(weight, torch.Tensor):
            weight = weight.permute([0, 2, 1, 3]).contiguous().view(self.out_channel_pad, self.in_channel_pad)
