...
def sparse_bp_conv2d(
    ...
    Args:
        x (torch.Tensor): padded input feature maps [bs, inc, h, w]
        u (torch.Tensor): U matrices. [p, q, k, k]
        s (torch.Tensor): Sigma matrices. [p, q, k]
        v (torch.Tensor): V* matrices. [p, q, k, k]
        weight_size: (_size): weight_size
        stride (int or Tuple[int]) conv stride
        padding (int or Tuple[int]) conv padding
        dilation (int or Tuple[int]) conv dilation
        groups (int or Tuple[int]) conv groups
    ...
    Returns:
        [torch.Tensor]: [bs, outc, h_out, w_out] output tensor
    """
    outc, inc, kernel_size = weight_size[:3]

    class SparseBPConv2dFunction(torch.autograd.Function):
        @staticmethod
        def forward(ctx, x: Tensor, u: Tensor, s: Tensor, v: Tensor) -> Tensor:
            ...
            return out  # [bs, outc, h_out, w_out]

        @staticmethod
        def backward(ctx, grad_output: Tensor) -> Tuple[Tensor, ...]:
            ## grad_output [bs, outc, h_out, w_out]
            ...
            if ctx.needs_input_grad[2]:
                if feature_sampler is not None:
                    ...
                    if column_mask is not None:
                        ...
                        # grad_output[..., ~column_mask].fill_(0) ## not working !!
...
class MZIBlockConv2d(torch.nn.Module):
    ...
    def __init__(
        ...
        self.bp_feedback_sampler = FeedbackSampler(0, 0, alg="topk", mode="conv")
