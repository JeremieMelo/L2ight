...
from pyutils.mzi_op import (checkerboard_to_vector,
...
class SparseBP_Base(nn.Module):
        ...
        for layer in self.modules():
            if(isinstance(layer, (MZIBlockLinear, MZIBlockConv2d))):
                if(layer.U is not None):
                    layer.U.data.copy_(
                        projection_matrix_to_unitary(layer.U.data))
                    layer.V.data.copy_(
                        projection_matrix_to_unitary(layer.V.data))
        ...
        for layer_name, layer in self.fc_layers.items():
            if(layer.U is not None and layer.U.requires_grad == True):
                if(layer_name not in self.unitary_loss_cache):
                    eye_U, eye_V = self.unitary_loss_cache[layer_name] = [torch.eye(layer.U.size(
                        0), device=layer.U.device), torch.eye(layer.V.size(0), device=layer.V.device)]
                else:
                    eye_U, eye_V = self.unitary_loss_cache[layer_name]
                loss_list.extend([F.mse_loss(torch.matmul(layer.U, layer.U.t()), eye_U), F.mse_loss(
                    torch.matmul(layer.V, layer.V.t()), eye_V)])
    ...
    def gen_mixedtraining_mask(self, sparsity: float, prefer_small: bool = False, random_state: Optional[int] = None) -> Dict[str, Dict[str, Tensor]]:
        return {layer_name: layer.gen_mixedtraining_mask(sparsity, prefer_small, random_state) for layer_name, layer in self.named_modules() if isinstance(layer, (MZIBlockLinear, MZIBlockConv2d))}
    ...
    def get_power(self, mixedtraining_mask: Optional[Tensor] = None) -> float:
        power = sum(layer.get_power(
            mixedtraining_mask[layer_name]) for layer_name, layer in self.fc_layers.items())
        return power
    ...
    def gen_uniform_gradient_mask(self, bp_feedback_sparsity: Optional[float] = None) -> None:
        for layer in self.fc_layers.values():
            layer.gen_uniform_gradient_mask(
                bp_feedback_sparsity=bp_feedback_sparsity)
    ...
    def set_bp_feedback_sampler(self, forward_sparsity: float, backward_sparsity: float, alg: str = "topk", normalize: bool = False, random_state: Optional[int] = None):
        for layer in self.modules():
            # if(isinstance(layer, (MZIBlockLinear, MZIBlockConv2d))):
    ...
    def set_bp_input_sampler(self, sparsity: float, spatial_sparsity: float, column_sparsity: float, normalize: bool = False, random_state: Optional[int] = None, sparsify_first_conv: bool = True):
        counter = 0
        for layer in self.modules():
        ...
        for mode in ["conv", "linear"]:
            ...
            for layer in self.modules():
                if(isinstance(layer, MZIBlockConv2d if mode == "conv" else MZIBlockLinear)):
                    delta_list_U.append(
                        layer.delta_list_U.data.flatten(0, 1))  # [p,q,k]
                    # [p,q,k(k-1)/2]
                    ...
                    delta_list_V.append(
                        layer.delta_list_V.data.flatten(0, 1))  # [p,q,k]
                    # [p,q,k(k-1)/2]
            ...
            delta_list_U_dict[mode], phase_U_dict[mode], phase_bias_U_dict[mode], S_dict[mode], delta_list_V_dict[mode], phase_V_dict[
                mode], phase_bias_V_dict[mode] = delta_list_U, phase_U, phase_bias_U, S, delta_list_V, phase_V, phase_bias_V

        return delta_list_U_dict, phase_U_dict, phase_bias_U_dict, S_dict, delta_list_V_dict, phase_V_dict, phase_bias_V_dict
    ...
    def stack_all_phase_quantizer_dict(self) -> Tuple[PhaseQuantizer, PhaseQuantizer, PhaseQuantizer]:
        ...
        for mode in ["conv", "linear"]:
            phase_U_quantizer = PhaseQuantizer(self.w_bit, self.v_pi, self.v_max,
                                               self.gamma_noise_std, self.crosstalk_factor, 5, mode="rectangle", device=self.device)
            phase_S_quantizer = PhaseQuantizer(self.w_bit, self.v_pi, self.v_max,
                                               self.gamma_noise_std, self.crosstalk_factor, 3, mode="diagonal", device=self.device)
            phase_V_quantizer = PhaseQuantizer(self.w_bit, self.v_pi, self.v_max,
                                               self.gamma_noise_std, self.crosstalk_factor, 5, mode="rectangle", device=self.device)
            noisy_gamma_U, noisy_gamma_S, noisy_gamma_V = [], [], []
            for layer in self.modules():
                if(isinstance(layer, MZIBlockConv2d if mode == "conv" else MZIBlockLinear)):
                    noisy_gamma_U.append(
                        layer.phase_U_quantizer.noisy_gamma.data.flatten(0, 1))
                    noisy_gamma_S.append(
                        layer.phase_S_quantizer.noisy_gamma.data.flatten(0, 1))
                    noisy_gamma_V.append(
                        layer.phase_V_quantizer.noisy_gamma.data.flatten(0, 1))
            ...
            phase_U_quantizer.set_gamma_noise(
                self.gamma_noise_std, noisy_gamma_U.size())
            phase_S_quantizer.set_gamma_noise(
                self.gamma_noise_std, noisy_gamma_S.size())
            phase_V_quantizer.set_gamma_noise(
                self.gamma_noise_std, noisy_gamma_V.size())
    ...
    def identity_calibration_zgd(self,
        ...
        # delta_list_U = delta_list_U[0:1]
        # delta_list_V = delta_list_V[0:1]
        # phase_U = phase_U[0:1]
        # phase_V = phase_V[0:1]
        # phase_bias_U = phase_bias_U[0:1]
        # phase_bias_V = phase_bias_V[0:1]
        S = torch.linspace(0.1, 1, S.size(-1),
                           device=self.device).unsqueeze(0)  # [1, k]
        x = 1 / S  # Sigma^-1
        ...
        U = build_unitary(delta_list_U, phase_U,
                          phase_bias_U, phase_U_quantizer)
        V = build_unitary(delta_list_V, phase_V,
                          phase_bias_V, phase_V_quantizer)

        def model(x, phase_U=None, phase_V=None, U=None, V=None):
            if(phase_U is not None):
                Ur = build_unitary(delta_list_U, phase_U,
                                   phase_bias_U, phase_U_quantizer)
            else:
                Ur = U
            if(phase_V is not None):
                Vr = build_unitary(delta_list_V, phase_V,
                                   phase_bias_V, phase_V_quantizer)
            else:
                Vr = V
            # logger.info(Ur.size(), S.size(), Vr.size(), x.size())
            # [bs*p*q, k, k]
        ...
        def lr_scheduler(epoch_list):
            # return lr_min + 0.5 * (init_lr - lr_min) * (1+np.cos(np.pi*step/n_epochs))
        ...
        def restart(loss, epoch_list, phase_U, phase_V):
            ...
            #     phase_U[mask] = torch.randn_like(phase_U[mask])
            #     torch.random.manual_seed(np.random.randint(0, 1000))
            #     phase_V[mask] = torch.randn_like(phase_V[mask])
        ...
        metric_U_avg, metric_U_std, metric_U_min, metric_U_max = metric_U.mean(
        ).item(), metric_U.std().item(), metric_U.min().item(), metric_U.max().item()
        metric_V_avg, metric_V_std, metric_V_min, metric_V_max = metric_V.mean(
        ).item(), metric_V.std().item(), metric_V.min().item(), metric_V.max().item()
        # thres = torch.quantile(metric_U, 0.995)
        # logger.info(metric_U[metric_U > thres])
        logger.info(
        ...
        mlflow.log_metrics({
            "iloss_avg": loss.data.mean().item(), "iloss_std": loss.std().item(), "iloss_min": loss.min().item(), "iloss_max": loss.max().item(),
            "imetric_u_avg": metric_U_avg, "imetric_u_std": metric_U_std, "imetric_u_min": metric_U_min, "imetric_u_max": metric_U_max,
            "imetric_v_avg": metric_V_avg, "imetric_v_std": metric_V_std, "imetric_v_min": metric_V_min, "imetric_v_max": metric_V_max,
            "icore_call": total_step * best_U.size(0)}, step=total_step)

        # dist = SphereDistribution(lr_min, torch.zeros_like(phase_U), scale_tril=None)
        sigma = 0.1
        ...
        with TimerCtx() as t:
            while((epoch_list < n_epochs).any().item()):
                ...
                for _ in range(phase_U.size(-1)):
                    grad = 0
                    for _ in range(n):
                        delta_phase = sample_perturbation(
                            [2]+list(phase_U.shape))
                        phase_U_perturb = phase_U + delta_phase[0]
                        phase_V_perturb = phase_V + delta_phase[1]
                        cur_loss = obj_fn(
                            model(x, phase_U=phase_U_perturb, phase_V=phase_V_perturb), target)
                        total_step += 1
                        loss_diff = c * (cur_loss - loss)

                        grad += 1/n * delta_phase * \
                            loss_diff.unsqueeze(-1).unsqueeze(0)
                    m = 0.9 * m + lr.unsqueeze(-1).unsqueeze(0) * grad
                    # logger.info(grad_U.abs().mean().item())
                    phase_U -= m[0]
                    phase_V -= m[1]
                ...
                if(adaptive):
                    ...
                    loss = obj_fn(model(x, phase_V=phase_V,
                                        phase_U=phase_U), target)
        ...
        for layer in self.modules():
            if(isinstance(layer, (MZIBlockConv2d, MZIBlockLinear))):
                ...
                layer.I_U = torch.nn.Parameter(
                    best_U[i:i+p*q].clone().view(p, q, k, k), requires_grad=False)  # can be saved in checkpoint
    ...
    def identity_calibration_zcd(self,
                                 lr: Optional[float] = 0.1,
                                 n_epochs: Optional[int] = 400,
                                 lr_gamma: Optional[float] = 0.99,
                                 lr_min: Optional[float] = 0.006,
    ...
    def identity_calibration_ztp(self,
                                 lr: Optional[float] = 0.1,
                                 n_epochs: Optional[int] = 400,
                                 lr_gamma: Optional[float] = 0.99,
                                 lr_min: Optional[float] = 0.006,
        ...
        with TimerCtx() as t:
            for epoch in range(n_epochs):
                ...
                for i in range(n_step):
                    ...
                    loss_neg = obj_fn(model(x, phase_U=phase, V=V), target)
                    total_step += 1
                    loss, min_indices = torch.stack(
                        [loss_neg, loss, loss_pos], dim=-1).min(dim=-1)
    ...
    def identity_calibration(self,
        ...
        if(alg == "zgd"):
            ...
            raise ValueError(
                "Only support (zgd, ztp, zcd), but got alg = {alg}")

    def parallel_mapping_zgd(self,
                             lr: Optional[float] = 0.1,
                             n_epochs: Optional[int] = 400,
                             lr_gamma: Optional[float] = 0.99,
                             lr_min: Optional[float] = 0.006,
        ...
        def commit(best_phase_U, best_phase_V, best_S):
            ...
            for layer in self.modules():
                if(isinstance(layer, (MZIBlockConv2d, MZIBlockLinear))):
                    ...
                    layer.phase_U.data.copy_(
                        best_phase_U[i:i+p*q].view_as(layer.phase_U))
                    layer.phase_V.data.copy_(
                        best_phase_V[i:i+p*q].view_as(layer.phase_V))
                    Sigma = best_S[i:i+p*q]
                    S_max = Sigma.abs().max(dim=-1, keepdim=True)[0]
                    layer.phase_S.data.copy_(
                        Sigma.div(S_max).acos().view_as(layer.phase_S))
        ...
        def validate_and_log():
            if(validate_fn is not None):
                ...
                mlflow.log_metrics({"mloss_avg": best_loss.mean().item(
                ), "mloss_std": best_loss.std().item(), "mloss_min": best_loss.min().item(), "mloss_max": best_loss.max().item(), "mmetric_avg": norm_err.mean().item(), "mmetric_std": norm_err.std().item(), "mmetric_min": norm_err.min().item(), "mmetric_max": norm_err.max().item(), "macc": accv[-1].item(), "mcore_call": total_step * best_phase_U.size(0)}, step=total_step)
        validate_and_log()
        with TimerCtx() as t:
            for epoch in range(n_epochs):
                # Step 1: optimize U
                ...
                if(verbose and (epoch % 20 == 0 or epoch == n_epochs - 1)):
                    ...
                    # logger.info(U[0].diag())
                    validate_and_log()

        # step 2: project S only at the last step and only when identity has good quality

        # diag(I_U* x V* x W* x U x I_V)
        # logger.info(I_U.size(), V.size(), target_T.size(), U.size(), I_V.size())
        U = build_unitary(delta_list_U, best_phase_U,
                          phase_bias_U, phase_U_quantizer)
        V = build_unitary(delta_list_V, best_phase_V,
                          phase_bias_V, phase_V_quantizer)
        validate_and_log()
        if(ideal_I):
            S = U.mul(target.matmul(V.transpose(-1, -2))
                      ).sum(dim=-2)  # [bs, k]
        else:
            S = I_U.mul(V.matmul(target_T.matmul(U.matmul(I_V)))).sum(
                dim=-2)  # [bs, k]
    ...
    def parallel_mapping_zcd(self,
        ...
        with TimerCtx() as t:
            while((epoch_list < n_epochs).any() == True):
                ...
                lr = lr_scheduler(epoch_list)
                if(adaptive):
                    restart(criterion(model(U=U, S=S, V=V)),
                            epoch_list, phase_U, phase_V, restart_thres)
                    restart_thres *= 1.02
    ...
    def parallel_mapping(self,
                         ...
                         verbose=True,
                         validate_fn: Callable = None,
                         ideal_I: Optional[bool] = False) -> None:
        if(alg == "zgd"):
    ...
    def get_learning_profiling(self, flat: Optional[bool] = False, input_size: Tuple = (32, 3, 32, 32)) -> Dict[str, Union[float, Dict]]:
        report = {
            "total":
            {
                "core_call": 0,
        ...
        breakdown = report["breakdown"] = sum(m.profiler for m in self.modules(
        ) if isinstance(m, (MZIBlockConv2d, MZIBlockLinear))).report
        total = report["total"]
        for k in total:
        ...
        def hook_fn(m, i, o):
            ...
            #     ### residual block is not sampled. Thus the total activation size for the skip connection and the first conv should sum to the original activation
            #     mem[m] = 0
            #     mem_sparse[m] = i[0].numel() * m.conv1_spatial_sparsity
            else:
                mem[m] = i[0].numel()
        ...
        if(flat):
            report = {f"{i}_{j}": report[i][j]
                      for i in report for j in report[i]}
        return report
