...
def set_sparsity(model: nn.Module) -> None:
    ...
    model.set_bp_feedback_sampler(float(configs.sparse.bp_forward_weight_sparsity),
                                  float(
        configs.sparse.bp_feedback_weight_sparsity),
...
def reset_sparsity(model: nn.Module) -> None:
     # reset sparsity
    model.set_bp_feedback_sampler(0,
                                  0,
                                  alg=configs.sparse.bp_feedback_alg,
                                  normalize=configs.sparse.bp_feedback_norm,
                                  random_state=None)
    model.set_bp_input_sampler(0,
                               0,
                               0,
                               normalize=configs.sparse.bp_input_norm,
                               random_state=None)
    model.set_bp_rank_sampler(configs.model.block_list[0], alg=configs.sparse.bp_rank_alg,
                              sign=0, random_state=None)
...
def extract_gradients(model: nn.Module):
    ...
    for name, m in model.named_modules():
        if(isinstance(m, (MZIBlockConv2d, MZIBlockLinear))):
            grads[name] = m.S.grad.data.clone().flatten(
                0)  # [p, q, k] -> [pqk]
...
def cosine_distance(grads_1: Dict[str, Tensor], grads_2: Dict[str, Tensor]) -> Dict[str, float]:
    ...
    for idx, name in enumerate(grads_1):
        dist[idx] = (1-F.cosine_similarity(grads_1[name],
                                           grads_2[name], dim=0)).cpu().item()
    return dist

    # return {1-F.cosine_similarity(grads_1[name], grads_2[name]) for name in grads_1}


def l2_distance(grads_1: Dict[str, Tensor], total_grad_1: Tensor, grads_2: Dict[str, Tensor], total_grad_2: Tensor) -> Dict[str, float]:
    dist = np.zeros([len(grads_1)])
    for idx, name in enumerate(grads_1):
        dist[idx] = (grads_1[name].sub(grads_2[name]).square_().sum().div(
            grads_1[name].norm(2).square())).cpu().item()
    total_dist = (total_grad_1.sub(total_grad_2).square_().sum().div(
        total_grad_1.norm(2).square())).cpu().item()

    return dist, total_dist
    # return {grads_1[name].sub(grads_2[name]).square_().sum().div(grads_1[name].norm(2).square()) for name in grads_1}


def angular_similarity(grads_1: Dict[str, Tensor], total_grad_1: Tensor, grads_2: Dict[str, Tensor], total_grad_2: Tensor) -> Dict[str, float]:
    dist = np.zeros([len(grads_1)])
    for idx, name in enumerate(grads_1):
        dist[idx] = (1-F.cosine_similarity(grads_1[name],
                                           grads_2[name], dim=0).acos_().div_(np.pi)).cpu().item()
    total_dist = (1-F.cosine_similarity(total_grad_1,
                                        total_grad_2, dim=0).acos_().div_(np.pi)).cpu().item()
    return dist, total_dist
    # return {(1-F.cosine_similarity(grads_1[name], grads_2[name]).acos_())/np.pi for name in grads_1}
...
def train(
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if batch_idx % int(configs.run.log_interval) == 0:
            lg.info('Train Epoch: {} [{:7d}/{:7d} ({:3.0f}%)] Loss: {:.4f} Class Loss: {:.4f} angular: {:.3f} L2: {:.3f}'.format(
                epoch, batch_idx * len(data), len(train_loader.dataset),
                100. * batch_idx / len(train_loader), loss.data.item(), classify_loss.data.item(), total_arg_dist, total_l2_dist))
    ...
    layer_avg_arg_dist, layer_std_arg_dist = np.mean(
        accum_arg_dist, 0), np.std(accum_arg_dist, 0)
    layer_avg_l2_dist, layer_std_l2_dist = np.mean(
        accum_l2_dist, 0), np.std(accum_l2_dist, 0)
    total_avg_arg_dist, total_std_arg_dist = np.mean(
        accum_total_arg_dist), np.std(accum_total_arg_dist)
    total_avg_l2_dist, total_std_l2_dist = np.mean(
        accum_total_l2_dist), np.std(accum_total_l2_dist)
    mlflow.log_metrics({"total_avg_arg": total_avg_arg_dist, "total_std_arg": total_std_arg_dist,
                        "total_avg_l2": total_avg_l2_dist, "total_std_l2": total_std_l2_dist}, step=epoch)

    for i in range(len(layer_avg_arg_dist)):
        mlflow.log_metrics({f"l{i}_avg_arg": layer_avg_arg_dist[i], f"l{i}_std_arg": layer_std_arg_dist[i],
                            f"l{i}_avg_l2": layer_avg_l2_dist[i], f"l{i}_std_l2": layer_std_l2_dist[i]}, step=epoch)

    scheduler.step()
    accuracy = 100. * correct.float() / len(train_loader.dataset)
    lg.info(
        f"Train Accuracy: {correct}/{len(train_loader.dataset)} ({accuracy:.2f})%")
    mlflow.log_metrics({"train_acc": accuracy.data.item(),
...
def main() -> None:
    ...
    try:
        ...
        if(configs.noise.phase_bias):
            model.assign_random_phase_bias(
        ...
        for epoch in range(1, configs.run.n_epochs+1):
            ...
            # report = model.get_learning_profiling(flat=True)
